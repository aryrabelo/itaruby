...
run_gate() { # LABDIR CORPORA ART -> transcript on stdout, rc in $LAB/last-rc
  ( cd "$1" && PUBLIC_CORPORA_FILE="$2" PUBLIC_BASELINE_DIR="$1/scripts/baseline" \
...
for name in match wrong-rev missing default-skip; do
...
say 'accusation side — each half of the guard removed, its case must flip to SKIP'
...
for entry in "${mutants[@]}"; do
  ...
  RC=$(cat "$LAB/last-rc")
  if case_holds "$mcase" "$t" "$RC"; then
    bad "mutant $mname NOT accused: case $mcase still behaves as declared (rc=$RC)"
    continue
  fi
