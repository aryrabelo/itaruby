# `instance_accessor: false` suppresses BOTH instance sides. At runtime
# `Config.endpoint` works and `Config.new.endpoint` raises NoMethodError, so
# inventing the instance reader here would hide a real error: the option has
...
class Config
  mattr_accessor :endpoint, instance_accessor: false
end
